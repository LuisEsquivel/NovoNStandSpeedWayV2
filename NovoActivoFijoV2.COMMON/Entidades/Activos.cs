using System;
using System.Collections.Generic;
using System.Text;

namespace COMMON.Entidades
{
    public class Activos : BaseDTO
    {

        public string Descripcion { get; set; }
        public string CentroCostosID { get; set; }
        public string Edificio { get; set; }
        public string NoSerie { get; set; }
        public string Barcode { get; set; }
        public string UbicacionID { get; set; }
        public string FormaAdquisicionID { get; set; }
        public string FechaAdquisicion { get; set; }
        public string DocumentoCompra { get; set; }
        public decimal Costo { get; set; }
        public string Epc { get; set; }
        public string IdentificadorActivo { get; set; }
        public string Marca { get; set; }
        public string Modelo { get; set; }
        public string Piso { get; set; }
        public decimal DepresiacionAcumulada { get; set; }
        public decimal ValorenLibros { get; set; }
        public string Adicional { get; set; }
        public string Adicional2 { get; set; }
        public string Adicional3 { get; set; }
        public string Adicional4 { get; set; }
        public string Adicional5 { get; set; }
        public string Adicional6 { get; set; }
        public string Adicional7 { get; set; }
        public string Adicional8 { get; set; }
        public string Adicional9 { get; set; }
        public string Adicional10 { get; set; }
        public decimal Adicional11 { get; set; }
        public decimal Adicional12 { get; set; }
        public decimal Adicional13 { get; set; }
        public decimal Adicional14 { get; set; }
        public DateTime Adicional15 { get; set; }
        public DateTime Adicional16 { get; set; }

    }
}
